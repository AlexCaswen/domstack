...
export default defineContentScript({
  ...
  main() {
    ...
    function isEnabled(): Promise<boolean> {
      return new Promise((resolve) => {
        if (chrome?.storage?.sync) {
          chrome.storage.sync.get({ enabled: true }, (r) => resolve(Boolean(r.enabled)));
        } else {
          resolve(true);
        }
      });
    ...
    function findDismissButton(container: HTMLElement): HTMLElement | null {
    ...
    function tryBackdropDismiss(modal: HTMLElement): void {
